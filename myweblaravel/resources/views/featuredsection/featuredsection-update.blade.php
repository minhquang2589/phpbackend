...
@section('title', 'Update Featured Section')
<!--  -->
@section('content')
<div class="">
         ...
         <tbody class=" divide-gray-200">
            <tr>
               <td class=" text-gray-900">
                  <div class="w-28 h-28">
                     <img src="{{ asset('images/' . $featuredsectionsEdit->image) }}" alt="{{ $featuredsectionsEdit->name }}" class="object-cover transition duration-500 group-hover:scale-105" />
                  </div>
               </td>
               <td>{{ $featuredsectionsEdit -> name}}</td>
               <td>
                  @if ($featuredsectionsEdit -> status)
   ...
   <div class="lg:flex justify-center">
      <div class="lg:w-1/2 mx-4 ">
         ...
         @endif
         <form action="{{ route('featuredsection.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="">
               <div class="relative z-0 mb-2 group">
                  <input type="text" name="content" value="{{$featuredsectionsEdit->name}}" id="content" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" />
            ...
            <label>
            <input class="size-4 rounded border-gray-300" type="checkbox" name="status" {{ $featuredsectionsEdit->status == 1 ? 'checked' : '' }}>
               <span class="text-red-600 ml-3">Status</span>
            </label>
            <input type="hidden" name="id" value="{{$featuredsectionsEdit->id}}">
